...
int32_t romiReadFeedbackPacket(uint8_t* packetBuffer, uint8_t len){
  ...
  while(1){
    switch(state){
      case wait_until_AA:
        ...
        if(status != NRF_SUCCESS) {
          printf("UART error reading start of romi header: %d\n", status);
          if (aa_count++ < 20) {
            printf("\ttrying again...\n");
            break;
          } else {
        ...
        if(status != NRF_SUCCESS) {
          printf("UART error reading last of romi header: %d\n", status);
          if (aa_count++ < 20) {
            printf("\ttrying again...\n");
            break;
          } else {
