...
static int32_t romiSendPayload(uint8_t* payload, uint8_t len) {
...
int32_t romiDriveDirect(int16_t leftWheelSpeed, int16_t rightWheelSpeed){
	int32_t CmdSpeed;
	int32_t CmdRadius;
	...
	if (rightWheelSpeed == leftWheelSpeed) {
	    CmdRadius = 0;  // Special case 0 commands Romi travel with infinite radius.
	} else {
	...
	int32_t status = romiDriveRadius(CmdRadius, CmdSpeed);


	return status;
}

int32_t romiDriveRadius(int16_t radius, int16_t speed){
    uint8_t payload[6];
    payload[0] = 0x01;
    payload[1] = 0x04;
    ...
    return romiSendPayload(payload, 6);
}
...
int32_t romiSetControllerDefault(void) {
    ...
    return romiSendPayload(payload, 15);
}
...
/*int32_t romiPlaySound(uint32_t frequency_hz, uint8_t duration_ms) {
    ...
    return romiSendPayload(payload, 5);
}*/
...
int32_t romiPlaySoundSequence(RomiSound_t sound) {
    ...
    return romiSendPayload(payload, 3);
}
...
int32_t romiRequestInformation(void) {
    ...
    return romiSendPayload(payload, 4);
}
