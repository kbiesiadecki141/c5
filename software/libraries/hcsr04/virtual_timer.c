...
// void handle_expired(void){
//   node_t* first = list_get_first();
//   while(first && first->timer_value < read_timer()){
//     first->callback_function();
//     if(first->repeated){
//       first->timer_value += first->microseconds;
...
//   struct node_t* this_timer  = (struct node_t*) malloc(sizeof(struct node_t));
//   this_timer->callback_function = cb;
//   this_timer->id = (uint32_t)this_timer;
//   this_timer->repeated = repeated;
//   this_timer->microseconds = microseconds;
//   this_timer->timer_value = read_timer() + microseconds;

//   list_insert_sorted(this_timer);

//   handle_expired();

//   return (uint32_t)this_timer;
// }
